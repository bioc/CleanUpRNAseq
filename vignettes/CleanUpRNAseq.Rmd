...
```{r queryannotationhub, eval=TRUE}
...
gtf <- system.file("extdata", "example.gtf.gz",
                    package = "CleanUpRNAseq")
hs_ensdb_sqlite_toy <- make_ensdb(
    gtf = gtf,
    outfile = file.path(
        tmp_dir,
...
gunzip(file.path(tmp_dir, gtf), remove = TRUE, overwrite = TRUE)
...
invisible({
    capture.output({
        counts_list <- summarize_reads(
            ...
            strandSpecific = 0,
            saf_list = saf_list,
            gtf = file.path(tmp_dir, gsub(".gz", "", gtf)),
            threads = 1,
            verbose = FALSE
        )
    })
...
```
![Fig 1](./qc1_read_assign_stat.png).
